...
const INDEX_PERFORMANCE_THRESHOLD_SEC = Number(Cypress.env('INDEX_PERFORMANCE_THRESHOLD_SEC'));
const SEARCH_PERFORMANCE_THRESHOLD_SEC = Number(Cypress.env('SEARCH_PERFORMANCE_THRESHOLD_SEC'));
...
async function testPerformance(
  ...
  for (let j = 0; j < NUM_TEST_ITERATIONS; j++) {
    const octopus = await instance.create(
      ACCESS_KEY,
  ...
  const searchAvgPerf = searchPerfResults.reduce((a, b) => a + b) / NUM_TEST_ITERATIONS;
