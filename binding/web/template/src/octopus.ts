...
export class Octopus implements OctopusEngine {
   ...
   *
   * @param appId - appId generated by Picovoice Console
   *
   * @returns An instance of the Octopus engine.
   */
  public static async create(appId: string): Promise<Octopus> {
    const wasmOutput = await Octopus.initWasm(appId);
    return new Octopus(wasmOutput);
  }

  private static async initWasm(appId: string): Promise<OctopusWasmOutput> {
    ...
    let status = await pv_octopus_init(appIdAddress, objectAddressAddress);
    if (status !== PV_STATUS_SUCCESS) {
      throw new Error(
